...
ZG_DISTRICTS    = ["Srodmiescie", "Nowe Miasto", "Zacisze", "Stary Kisielin"]
DISTRICT_NICHES = ["hydraulik", "elektryk", "firma remontowa", "malarz pokojowy", "sprzatanie"]

GORZOW_DISTRICTS = ["Srodmiescie", "Zawarcie", "Staszica", "Siedlice"]
...
async def main():
    zg_queries = BASE_QUERIES + [
        f"{niche} {district}"
        for niche in DISTRICT_NICHES
        for district in ZG_DISTRICTS
    ]
    gorzow_queries = BASE_QUERIES + [
        f"{niche} {district}"
        for niche in DISTRICT_NICHES
        for district in GORZOW_DISTRICTS
    ]

    print("=" * 62)
    print("  Most Digital — Zielona Gora / Lubuskie Lead Collector")
    print(f"  Zielona Gora: {len(zg_queries)} zapytan")
    print(f"  Gorzow Wlkp:  {len(gorzow_queries)} zapytan")
    print(f"  Inne miasta:  {len(OTHER_CITIES)} x {len(BASE_QUERIES)}")
    ...
    print(f"\n>>> ZIELONA GORA ({len(zg_queries)} zapytan)")
    await scrape_city("Zielona Gora", zg_queries, seen, all_biz, "ZG")

    print(f"\n>>> GORZOW WIELKOPOLSKI ({len(gorzow_queries)} zapytan)")
    await scrape_city("Gorzow Wielkopolski", gorzow_queries, seen, all_biz, "GOR")
    ...
    try:
        n_new = sync_leads(all_biz, city="Lubuskie", source_query="zielona_gora_batch")
